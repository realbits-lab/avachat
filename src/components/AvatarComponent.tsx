...
export default function AvatarComponent({
  registerData,
  hidePriceButton = false,
}: AvatarProps) {
  //* You can divide free and commercial item based on this rentFee.
  const rentFee = (registerData?.rentFee || 0) / Math.pow(10, 18);
  //* TODO: Consider token decimal.
  const rentFeeByToken = (registerData?.rentFeeByToken || 0) / Math.pow(10, 18);
  ...
  const {
    ...
    address: registerData?.nftAddress as `0x${string}`,
    ...
    return (
      <>
        ...
        <Button
          ...
          onClick={async () => {
            writeRentNFT?.({
              ...
              value: BigInt(registerData?.rentFee || 0),
            });
          }}
        >
          {formatEther(BigInt(registerData?.rentFee || 0))} matic
        </Button>
        ...
        <Button
          ...
          variant="outlined"
          disabled={
            registerData?.rentFee === 0 && registerData?.rentFeeByToken === 0
          }
          onClick={async () => {
            const contract = getContract({
              address: registerData?.feeTokenAddress as Address,
        ...
        >
          {formatEther(BigInt(registerData?.rentFeeByToken || 0))} token
        </Button>
      </>
    );
  }
  ...
  return (
    ...
    >
      ...
      <div className="py-2 px-4">
        ...
        {hidePriceButton === false && <PriceButton />}
      </div>
    </div>
  );
}
